void run_knob_routine()
{
  ...
  // int tbutton1 = transport_button_1.despressed();
  knob_mode_select.Update();

  if(knob_mode_select.clicks == 1){
  ...
  if(knob_mode == 2) // Tempo adjust
  {
    ...
    if(knobs_cleared_to_update[0] == 1)
    {
      ...
      the_tempo_adjust_float_value = round(the_tempo_adjust_value/100);
      ...
      if((the_tempo_adjust_value >= (last_tempo_adjust_value+10)) || (the_tempo_adjust_value <= (last_tempo_adjust_value-10)))
      {
        ...
        the_serial_message += the_mapped_tempo_adjust_value;
        the_serial_message += ";";
        serial_printer(the_serial_message);
