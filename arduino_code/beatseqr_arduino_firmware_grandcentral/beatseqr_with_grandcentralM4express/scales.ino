// scales.ino
//
// Musical scale support for beatseqr.
//
...
// 16 patterns and 8 voices. Re-arms all pickup guards so sliders must cross
// the new quantized pitch before writing.
void apply_scale_to_all_patterns() {
  build_scale_notes();
  if (scale_note_count == 0) return;

  for (uint8_t p = 0; p < 16; p++) {
    for (uint8_t v = 0; v < VOICE_COUNT; v++) {
      voice_pitch[p][v] = quantize_to_scale(voice_pitch[p][v]);
    }
  }
