  ...
  if(dpad_down.uniquePress()){
    ...
    case 3:
      { // midi note number values
        ...
        // I would like to be able to change these values on the fly.
        // I need a combo move here!
        // like the OMGWTF button and step button 5.

        // if there's no saved range values, set the defaults
        if(midinn_sliderrangelow = -1)
        {
          midinn_sliderrangelow = 0;
        }

        if(midinn_sliderrangehigh = -1)
        {
          midinn_sliderrangehigh = 128;
        }
