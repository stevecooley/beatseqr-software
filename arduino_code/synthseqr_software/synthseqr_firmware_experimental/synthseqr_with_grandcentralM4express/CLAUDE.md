...
| `midi_processor.ino`                       | MIDI input: clock sync (0xF8), start (0xFA), stop (0xFC); CC Learn; keyboard-programming note capture                                                                                                              |
...
| `clock_div.ino`                            | Global clock divide/multiply: `CLOCK_DIV[]` preset table + `apply_clock_div()` (pushes pulses-per-step into `seq.setPulsesPerStep()` and `clock_pulses_per_step` for the external clock path)                      |
...
int8_t  step_custom_chord[16][16][MAX_CHORD_NOTES]  // per-step captured chord pitches from external MIDI keyboard programming; slot 0 = lowest pitch (sorted), -1 = empty. When slot 0 >= 0 the step plays these literal pitches (still subject to octave/note shift + scale snap + drift) and step_chord_type is ignored for that step.
int8_t  midi_capture_step             // -1 idle; else step index currently being programmed via held-step + keyboard
int8_t  midi_capture_buf[MAX_CHORD_NOTES]  // sorted (ascending), deduped capture buffer
uint8_t midi_capture_count            // count of valid entries in midi_capture_buf
uint8_t midi_capture_first_velocity   // velocity of the first note received; used as step velocity on commit
bool    midi_capture_dirty            // true if any note has been captured during the current step hold
uint8_t midi_capture_held[16]         // 128-bit set: bit (pitch & 7) of byte (pitch >> 3) — currently-held keyboard notes
int8_t  midi_capture_lcd_step         // step index for case 204 "Cap S{s} N{n}" line-2 feedback (-1 idle)
uint8_t midi_capture_lcd_count        // note count for case 204 feedback
...
uint8_t dpad_main_mode                // d-pad up/down target on main screen; 0=Pattern(default) 1=Oct 2=NoteShift 3=Tempo 4=Swing 5=PitchDrift 6=PatLen 7=PatDir 8=MIDIch 9=LvCCch 10=CC# 11=Chord 12=ClkDiv
...
uint8_t clock_div                     // index into CLOCK_DIV[] presets (config.h); default CLOCK_DIV_DEFAULT = 1/16
uint8_t clock_pulses_per_step         // derived cache of pulses-per-step used by the external-clock path; default 6 (1/16). Kept in sync by apply_clock_div()
uint8_t beat_pulse_count             // 0..23 (one quarter note) — play-LED beat flash counter, advanced in the midi() clock callback while playing
...
Global step-length control gated by `ft_clock_div` (default ON). The clock always runs at **24 PPQN**; the lever is **pulses-per-step**, not the timer period — so MIDI clock output stays a correct 24 PPQN and external gear stays in sync at all divisions.

- `CLOCK_DIV[]` (in `config.h`) is a fixed `{name, pulses}` preset table, ordered fastest→slowest: `1/32`(3) `1/16T`(4) `1/16`(6, default) `1/8T`(8) `1/8`(12) `1/4T`(16) `1/4`(24) `1/2`(48) `whole`(96). `CLOCK_DIV_COUNT` / `CLOCK_DIV_DEFAULT` (=2, the 1/16 index) bound the index. `T` entries are triplet feels (non-power-of-two pulse counts).
- `clock_div` is the active index; `apply_clock_div()` (in `clock_div.ino`) resolves it to pulses-per-step (forced to **6** when `ft_clock_div` is off) and pushes that into **both** clock paths: `seq.setPulsesPerStep()` (internal TC4 stepping) and the `clock_pulses_per_step` cache read by the external-clock step boundary in `midi_processor.ino`.
- `FifteenStep::hardwareClockPulse()` advances a step every `_pulses_per_step` pulses (was a hard-coded `6`); `setPulsesPerStep()` clamps to ≥1 and zeroes the in-progress pulse count so a mid-play change lands cleanly on the next boundary.
- Call `apply_clock_div()` after boot-load (in `setup()`), on the config-menu editor change, and when toggling `ft_clock_div` (both enable and disable) in the Features submenu.
- **Gates scale for free**: `step_gate` and `sounding_note_end_step` are measured in steps, so a longer division makes every gate proportionally longer (gate 1 at 1/4 = a full quarter note; a 16-step gate at 1/4 = 4 bars). Swing, chaining, and pattern length/direction are likewise step-based and unaffected.
- Editable from `Config Menu → Clk div` and bindable to main-screen d-pad via `DPAD_MAIN_MODE_CLOCK_DIV`.

**Play-button LED beat flash**: the flash is driven by `beat_pulse_count` in the `midi()` clock callback (`midi_note_sending.ino`), **not** by step boundaries — so it blinks on the quarter-note beat regardless of `clock_div`. The callback fires once per 24-PPQN pulse while `playstatus` is true (in both clock modes, since both route through `_tick()`), turning the LED on for `beat_pulse_count < 12` and off otherwise, wrapping at 24. `beat_pulse_count` is reset to 0 at each play-start (both transport branches in `transport.ino` and the external deferred-start in `midi_processor.ino`) for phase alignment. The old `current_step % 4` flash in `stepsend()` was removed.
...
## MIDI Keyboard Programming

Hold a step button + play notes on a USB-MIDI keyboard → the step captures the played notes as arbitrary pitches. Works in every slider mode. Gated by `ft_midi_program_mode` (default ON; toggle from Features → MIDI program).

**Capture flow** (`read_midi()` in `midi_processor.ino`):
- Per-loop step-press scan in `detect_step_button_presses()` calls `midi_capture_arm(i)` at the top of each `uniquePress` branch. This sets `midi_capture_step = i` and clears the buffer.
- `read_midi()` branches on USB-MIDI CIN 0x09 (note-on) and 0x08 (note-off). Channel is ignored.
- Note-on (vel > 0): insertion-sort by pitch into `midi_capture_buf` (ascending, deduped, capped at `MAX_CHORD_NOTES`); update `midi_capture_held` bit; set `midi_capture_first_velocity` if this is the first note; set `midi_capture_dirty = true`. **Fresh-strum-replaces-buffer rule**: if the held-keys mask was empty before this note AND the buffer is non-empty, the buffer is wiped first — so "lift everything and replay" overrides the previous strum during the same step hold.
- Note-off (or note-on with vel == 0): clear the held-keys bit. The buffer is NOT cleared on note-off — capture accumulates the maximum set across the entire hold session.

**Commit timing**: capture commits on step-button release, NOT on every "all keys released." The release check at the top of `detect_step_button_presses()` runs each loop. When `midi_capture_step >= 0 && !step_buttons[midi_capture_step].wasPressed()`:
- If `midi_capture_dirty`, `midi_capture_commit(step)` writes the buffer to `step_custom_chord[pattern][step]`, promotes the lowest captured pitch into `voice_slider_midinotenum[step]` + `pattern_step_pitches[pattern][step]`, writes `midi_capture_first_velocity` into `pattern_step_velocities[pattern][step]`, resets `step_chord_type[pattern][step] = 0` (custom-chord branch wins anyway, but keeps state clean), activates the step (`step_data[pattern][0][step] = 1`, LED on), arms `slider_needs_pickup[step] = true`, and seeds `slider_last_raw[step]` from the current ADC reading.
- `midi_capture_discard()` then clears all capture state.
- If the committed step is also the `gate_hold_step`, that field is cleared so the deferred-toggle release check doesn't fire after a commit.

**Slider pickup after commit (critical)**: `midi_capture_commit()` MUST set `slider_needs_pickup[step] = true` rather than `false`. The physical NN slider is almost never at the captured low pitch; without pickup the next slider read writes `voice_slider_midinotenum[step]` to the slider's physical value, triggering the custom-chord eraser in `voice_slider_routine.ino` within 20 ms and silently wiping the capture. Pickup behavior depends on takeover mode: Catch requires moving the slider through the captured pitch; Jump requires `JUMP_RAW_THRESHOLD` movement; Relative ignores pickup but still only writes on a non-zero `value_delta`.

**NN slider erases custom chord**: When the NN-mode slider write path updates `voice_slider_midinotenum[j]` and `pattern_step_pitches[pattern][j]`, it also clears `step_custom_chord[pattern][j][n]` to all -1. Moving the slider is the eraser — no separate gesture needed. Also fires in Relative mode for any non-zero `value_delta`. CH slider writes don't trigger the eraser because they write `step_chord_type`, not `voice_slider_midinotenum` — which is why CH mode can capture without the pickup-after-commit fix.

**Custom chord clears on step OFF**: `do_step_toggle()` zeroes `step_custom_chord[pattern][i]` along with `step_chord_type` when toggling a step off, so re-activating the step starts from a clean slate.

**Gate-set after capture**: hold step A, play chord, release notes, then (still holding A) tap step B to set gate length. `midi_capture_arm(B)` returns `bool` — if a different step had a pending dirty capture, it commits that capture first (turning A on with the chord) before arming B. The gate-set gesture then runs normally: `do_step_on(A)` is a no-op because A is already on, and `step_gate[pattern][A]` is set to the forward distance. A keeps its chord AND gets the new gate. The double-tap branch (second tap within 150 ms) consults the `just_committed_other` return so it doesn't toggle A back off after the commit just turned it on.

**Gate-set wins over capture for the destination step**: when the gate gesture fires, `midi_capture_discard()` clears any in-flight capture state for the second-tapped step. The held step's capture has already been committed by `midi_capture_arm` at the top of the loop.

**Octave/note shift + scale snap**: captured pitches are literal MIDI numbers from the keyboard. At playback time (and audition time) they get `octave_shift` + `note_shift` + scale snap applied the same way slider-stored pitches do, so global transforms behave consistently. If the user wants the captured chord to sound at the literal pitch they played, they disable shifts globally. Per-step drift applies to each captured note independently.

**MIDI program persistence**: `step_custom_chord[16][16][6]` is saved to SD JSON (per-pattern `"custom_chords": [[6 ints], ...16 inner arrays]`; -1 marks empty slots) and EEPROM (address 2123, 1536 bytes; -1 stored as `0xFF`, anything other than 0..127 falls back to -1 on load). `ft_midi_program_mode` is persisted to both (SD `"ft_midi_program_mode"`; EEPROM address 2122). The `custom_chords` per-pattern array is optional in JSON — older saves without it default to all -1 for every step. Bumping `EEPROM_MAGIC_VALUE` to 0xD4 forces old EEPROM saves to be ignored on first boot after the upgrade (SD saves still load cleanly).

**LCD feedback**: `midi_capture_commit()` sets `lcdflag = next_lcdflag = 204`. Case 204 in `run_LCD_update()` shows `"Cap S{s+1:02} N{count}"` on line 2 for 800 ms, then returns to `next_lcdflag = 255`.

**Always boots to NN mode** (`slider_mode = 1`). Mode is not saved to SD/EEPROM — it resets to NN on power-up.

**`resetSliders()`**: Resets all 16 steps' pitches to `slider_map_low_value`, velocities to 127, gates to 1, probabilities to 100, drift to off/0, chord types to 0, and clears `step_custom_chord` to all -1, for the current pattern.
...
5. **Clk div (Clk div:)** — enter editing sub-state; up/down cycles `clock_div` through the `CLOCK_DIV[]` presets (down=faster, up=slower); calls `apply_clock_div()` on change; label shows `*` when not 1/16. Hidden when `ft_clock_div` is off
6. **D-pad up/dn (D-pad:)** — enter editing sub-state; up/down cycles through enabled targets (Pattern / Octave / Note / Tempo / Swing / Drift / PatLen / PatDir / MIDIch / LvCCch / CC# / Chord / ClkDiv); Enter or Left exits editing. Selects what d-pad up/down does on the main screen. Pattern keeps the original step-feedback line 2; any other choice replaces line 2 with a persistent indicator showing the bound target's current value. Always visible.
7. **Diagnostics** — opens the Diagnostics submenu (LED test, Input test, Hi trim, Noise — see below). Hidden when `ft_diagnostics` is off
8. **Exit** — Enter, left, or right all exit
9. **Features** — enters the Features submenu (see below)
10. **Live CC ch** — enter editing sub-state; up/down adjust 1–16; Enter or Left exits editing. Sets the MIDI channel used by LV slider mode (independent of `MIDICHANNEL`). Hidden when `ft_live_cc_mode` is off
11. **Mode: Simple/Advanced** — confirmation required; Enter toggles Simple↔Advanced; line 1 shows target (`Mode:->Simple ` or `Mode:->Advancd`); Left cancels. Enabling Advanced also force-enables `ft_velocity_mode` and `ft_probability`. Hidden when `ft_advanced_mode` is off
12. **Note range** — opens Note range submenu (see below); label shows `*` when non-default (36/52). Always visible (not gated by any feature flag)
13. **Note scales** — two-phase editor: Enter starts editing scale type (`Sc: Major`), Enter again switches to root note (`Root: C#`), Enter exits; changing scale or root calls `apply_scale_to_all_patterns()` which quantizes all stored pitches immediately; label shows `*` when non-Chromatic/C. Scales: Chromatic Blues Dorian HarmMinor Major Mixolydian NatMinor PentMaj PentMin Phrygian. Hidden when `ft_scale_quantization` is off
14. **Note shift** — enter editing sub-state; up/down adjust ±1 semitone (range -12 to +12); label shows `*` when non-zero. Hidden when `ft_octave_note_shift` is off
15. **Octave shift** — enter editing sub-state; up/down adjust ±1 octave (range -5 to +5); label shows `*` when non-zero. Hidden when `ft_octave_note_shift` is off
16. **Pat dir** — enter editing sub-state; up/down cycles 0–7; names: Fwd/Rev/Pong/Rand/Shuf/E·O/In/Quad. Hidden when `ft_pattern_direction` is off
17. **Pat length** — enter editing sub-state; up/down adjust 1–16; tap any step button sets length to N+1; step LEDs show active length while editing; `seq.setSteps()` called on every change; label shows `*` when not 16. Hidden when `ft_variable_pat_length` is off
18. **Pitch drift** — enter editing sub-state; up/down adjust 0–7 semitones; label shows `*` when non-zero. Hidden when `ft_pitch_drift` is off
19. **Save** — saves to SD (primary) + EEPROM (backup); blocked while playing (shows `Stop first!` on line 2); exits menu and shows `saved!` on success
20. **Slider takeover (Takeover:)** — enter editing sub-state; up/down cycles Catch / Jump / Relative; line 1 shows current value. Changing the value re-seeds `slider_last_raw[]` from current ADC reads and clears/sets `slider_needs_pickup[]` according to the new mode. Always visible.
21. **Step prob** — exits menu immediately and activates PR slider mode; label shows `*` if any step probability < 100. Hidden when `ft_probability` is off
22. **Swing** — enter editing sub-state; up/down adjust 0–5; Enter or Left exits editing. Hidden when `ft_swing` is off
23. **Tempo** — only visible when external clock is OFF; Enter starts editing (line 2 shows resolution); Enter again cycles resolution ±10 → ±1 → ±0.1 BPM; up/down adjusts at current resolution; Left exits. Calls `seq.setTempo()` and `setSequencerTimerPeriod()` on every change
...
| MIDI program      | `ft_midi_program_mode`| Hold step + play USB-MIDI keyboard notes → capture chord onto step (default ON) |
| Clock div         | `ft_clock_div`        | Clock divide/multiply: Clk div menu item, DPAD_MAIN_MODE_CLOCK_DIV target, pulses-per-step in `apply_clock_div()` (default ON; stepping forced to 1/16 when off) |
  ...
  "patterns": [
    {
      ...
      "custom_chords":[[-1,-1,-1,-1,-1,-1], ...16 inner arrays of 6 ints]
    },
    ...16 patterns
  ]
}
...
`pitch_drift`, `probabilities`, `drift_enabled`, `drift_amounts`, per-pattern `chord_types`, and per-pattern `custom_chords` are optional in the JSON — older saves without them load cleanly and default to 0 / 100 / 0 / 0 / 0 / all-(-1) respectively. The global `ft_chord_mode`, `ft_midi_program_mode`, and `current_chord_type` keys are also optional (default to feature ON, single-note paint). The global `ft_clock_div` and `clock_div` keys are optional too (default to feature ON / 1/16 division).
...
**EEPROM layout** (3661 bytes, defined as `#define` constants in `storage.ino`; `EEPROM_EMULATION_SIZE = 4096` — ~435 bytes headroom remaining):
...
| 3659    | 1    | `clock_div` (uint8_t, index into `CLOCK_DIV[]`)        |
| 3660    | 1    | `ft_clock_div` (bool)                                  |

**Validation**: All loaded values are range-checked so corrupted flash can't break the sequencer. CC numbers validated to be in safe range (1–119, not 32, not 96–101). `pitch_drift` validated 0–7; `step_probability` validated 0–100; `step_drift_amount` validated 0–12; `step_drift_enabled` coerced to 0/1; `step_chord_type` and `current_chord_type` validated < `CHORD_COUNT` (out-of-range values default to 0); `slider_noise_threshold` accepted only if exactly 12, 24, or 48 (anything else keeps the default 24); `step_custom_chord` slots accepted if 0..127 (`0xFF` decoded as -1), anything else falls back to -1; `clock_div` validated < `CLOCK_DIV_COUNT` (else keeps default); `ft_clock_div` accepted only as 0/1.
...
**Magic byte**: Increment `EEPROM_MAGIC_VALUE` in `storage.ino` whenever the layout changes. Current value: `0xD5` (bumped from `0xD4` for `clock_div` + `ft_clock_div`). Existing EEPROM-only saves from before the bump are ignored on first boot after the upgrade and globals fall back to compiled defaults; SD saves still load cleanly because the new keys are all optional.
...
3. Source the chord: if `step_custom_chord[pattern_value][step][0] >= 0` use the stored captured pitches (each octave/note-shifted + scale-snapped); otherwise call `build_chord_pitches(root, step_chord_type[pattern_value][step], chord_out)` to produce 1..6 MIDI pitches. With `step_chord_type == 0` and no captured chord this is just the root, byte-identical to the pre-chord audition.
