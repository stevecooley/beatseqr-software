...
4. `stepsend()` first scans all 16 `sounding_note_end_step[]` entries — any step whose end-step equals `current_step` gets a note-off sent (supports multi-step gate lengths). Then, if the current step is active, note-on is sent using `pattern_step_pitches[pattern][current_step]` and `voice_slider_midivelocity[current_step]`; `sounding_note_end_step[current_step]` is set to `(current_step + step_gate[pattern][current_step]) % 16`. `last_triggered_step` is updated and `update_line2` is set for the LCD.
...
**Double-tap detection**: implemented in the main `loop()` with a static `last_enter_ms` timestamp. Two `uniquePress()` events within 400 ms trigger `enter_config_menu()` instead of setting `enterbutton_flag`. Single tap still behaves as before (slider mode cycle, enter LED toggle).
