...
bool load_from_sd() {
  ...
  _f.seekSet(0);
  if (sd_find("\"dpad_main_mode\":")) {
    int v = (int)sd_parse_number();
    if (v >= 0 && v < DPAD_MAIN_MODE_COUNT) dpad_main_mode = (uint8_t)v;
  }
  ...
  _f.seekSet(0);
  if (sd_find("\"current_chord_type\":")) {
    int v = (int)sd_parse_number();
    if (v >= 0 && v < (int)CHORD_COUNT) current_chord_type = (uint8_t)v;
  }
  ...
  for (int p = 0; p < 16; p++) {
    ...
    {
      uint32_t pos = _f.position();
      if (sd_find_bounded("\"chord_types\":", 400)) {
        if (sd_read_until('[')) {
          for (int s = 0; s < 16; s++) {
            sd_skip_ws();
            int v = (int)sd_parse_number();
            if (v >= 0 && v < (int)CHORD_COUNT) step_chord_type[p][s] = (uint8_t)v;
            sd_skip_ws();
            char c = (char)_f.peek();
            if (c == ',' || c == ']') _f.read();
          }
